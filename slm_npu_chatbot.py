...
class SLMNPUChatbot:
    ...
    def extract_keywords_from_question(self, question: str) -> Dict:
        ...
        if any(kw in question_lower for kw in ['가장 위험', '가장 심각', '최고 위험']):
            ...
            query_type = 'location'
            keywords.append('위치')
        elif any(kw in question_lower for kw in ['몇 개', '개수', '통계']):
    ...
    def answer_question(self, question: str) -> Dict:
        ...
        if not self.available:
            return {
                'answer': '죄송합니다. SLM NPU Worker가 연결되지 않았거나 모델이 로드되지 않았습니다.',
                'data': None,
                'image_path': None
            }
...
답변:"""
            ...
            response = requests.post(
                ...
                json={
                    'prompt': prompt,
                    'system_message': '당신은 도로 포트홀 탐지 시스템의 AI 어시스턴트입니다. 제공된 데이터를 바탕으로 사용자의 질문에 정확하고 친절하게 답변합니다.',
                    'max_tokens': 200,
                    'temperature': 0.7
                },
                timeout=60  # NPU에서 약간 더 오래 걸릴 수 있음
            )
